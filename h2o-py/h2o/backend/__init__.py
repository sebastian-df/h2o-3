...
if not hasattr(colorama, '__version__'):
    logger.warn("[WARNING] H2O requires colorama module of version 0.3.8 or newer. You have older version.")
    logger.warn("You can upgrade to the newest version of the module running from the command line")
    logger.warn("    $ pip%s install --upgrade colorama", sys.version_info[0])
    sys.exit(1)
elif StrictVersion(colorama.__version__) < StrictVersion("0.3.8"):
    logger.warn("[WARNING] H2O requires colorama module of version 0.3.8 or newer. You have version %s.")
    logger.warn("You can upgrade to the newest version of the module running from the command line")
    logger.warn("    $ pip%s install --upgrade colorama", colorama.__version__, sys.version_info[0])
    sys.exit(1)
